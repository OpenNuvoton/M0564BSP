...
SC_T *_scBase[SC_INTERFACE_NUM] = {SC0, SC1};
uint32_t vccDelay[SC_INTERFACE_NUM] = {0, 0};
...
int32_t SCLIB_Activate(uint32_t num, uint32_t u32EMVCheck)
{
    ...
    int32_t retval, clk;
    uint32_t reg = sc->INTEN;


    clk = SC_GetInterfaceClock(sc);
    if(clk < 1000 || clk > 5000)
        return SCLIB_ERR_CLOCK;

    sc->INTEN &= ~SC_INTEN_CDIEN_Msk;
...
void SCLIB_Deactivate(uint32_t num)
{
    ...
    while(sc->STATUS & SC_STATUS_TXACT_Msk) ;
    while(sc->STATUS & SC_STATUS_RXACT_Msk) ;
...
int32_t SCLIB_GetCardAttrib(uint32_t num, SCLIB_CARD_ATTRIB_T *s_attrib)
{
    ...
    s_attrib->conv = sc->CTL & SC_CTL_CONSEL_Msk ? 1 : 0;
...
uint32_t SCLIB_CheckCDEvent(uint32_t num)
{
    ...
    if(sc->INTSTS & SC_INTSTS_CDIF_Msk)        /* toggle detect by card */
    {
        SCLIB_INFO("SC%d: Card Present Int : SC_INTSTS : [%02x]\n", num, sc->INTSTS);
        sc->INTSTS = SC_INTSTS_CDIF_Msk; // clear CDIF bit
        reg = sc->STATUS;

        if((reg & (SC_STATUS_CREMOVE_Msk | SC_STATUS_CINSERT_Msk)) ==
                (SC_STATUS_CREMOVE_Msk | SC_STATUS_CINSERT_Msk))      // CINSERT & CREMOVE both trigger
        {
            SCLIB_INFO("SC%d: Card status not sure\n", num);
            // [2011.11.30]
            /* protect writing operation forever */
            sc->INTEN &= ~SC_INTEN_TXEIEN_Msk;
            sc->STATUS |= (SC_STATUS_CREMOVE_Msk | SC_STATUS_CINSERT_Msk);
        ...
        else if((reg & SC_STATUS_CINSERT_Msk) &&
                (((reg & SC_STATUS_CDPINSTS_Msk) >> SC_STATUS_CDPINSTS_Pos) == (((sc->CTL) & SC_CTL_CDLV_Msk) >> SC_CTL_CDLV_Pos)))
        {
            sc->STATUS |= SC_STATUS_CINSERT_Msk;   // clear CINSERT flag
            SCLIB_INFO("SC%d: Card Inserted\n", num);

            dev->bCardRemoved = SCLIB_CARD_PRESENT;

        }
        else if((reg & SC_STATUS_CREMOVE_Msk) &&
                (((reg & SC_STATUS_CDPINSTS_Msk) >> SC_STATUS_CDPINSTS_Pos) != (((sc->CTL) & SC_CTL_CDLV_Msk) >> SC_CTL_CDLV_Pos)))
        {
            sc->STATUS |= SC_STATUS_CREMOVE_Msk;   // clear CREMOVE flag
...
uint32_t SCLIB_CheckTimeOutEvent(uint32_t num)
{
    ...
    if((sc->INTSTS & SC_INTSTS_TMR0IF_Msk) && (sc->INTEN & SC_INTEN_TMR0IEN_Msk))
    {
        sc->INTSTS = SC_INTSTS_TMR0IF_Msk;   // Clear interrupt flag
        if(atr_final_chk == 1)
            atr_final_chk = 0;
        else
    ...
    if((sc->INTSTS & SC_INTSTS_TMR1IF_Msk) && (sc->INTEN & SC_INTEN_TMR1IEN_Msk))
    {
    ...
    if((sc->INTSTS & SC_INTSTS_TMR2IF_Msk) && (sc->INTEN & SC_INTEN_TMR2IEN_Msk))
    {
        sc->INTSTS = SC_INTSTS_TMR2IF_Msk;
...
uint32_t SCLIB_CheckTxRxEvent(uint32_t num)
{
    ...
    if(sc->INTSTS & SC_INTSTS_TXEIF_Msk)
    {
        retval = 1;
    ...
    if(sc->INTSTS & (SC_INTSTS_RDAIF_Msk | SC_INTSTS_RXTOIF_Msk))
    {
        retval = 1;
        ...
        else if(dev->op_state == SCLIB_OP_ATR_READ)      /* Read ATR ISR */
        {
            ...
            SC_StopTimer(sc, 0);
            /* [ISO 7816-3] */
            //SMARTCARD_TimerCountSet(sc, 0, SC_TMR_OPMODE_0, 9627);  // default waiting time 9600 ETUs
            /* [EMV2000] */
            ...
            if((sc->INTSTS & SC_INTSTS_RDAIF_Msk) && atr_remain)
            {
                /*
                 * atr_len==0 : TS
                 * atr_len==1 : T0
            ...
            if(atr_remain == 0)     /* receive ATR done */
            {
                ...
                SC_StopTimer(sc, 2);
                atr_final_chk = 1;
                if((sc->INTSTS & SC_INTSTS_RDAIF_Msk) && atr_final_chk)
                {
                    SC_StopTimer(sc, 0);
...
uint32_t SCLIB_CheckErrorEvent(uint32_t num)
{
    ...
    if(sc->INTSTS & SC_INTSTS_ACERRIF_Msk)
    {
        sc->INTSTS = SC_INTSTS_ACERRIF_Msk;
    ...
    if((sc->INTSTS & SC_INTSTS_TERRIF_Msk) && (sc->INTEN & SC_INTEN_TERRIEN_Msk))
    {
        if(sc->STATUS & SC_STATUS_RXOV_Msk)
        {
            sc->STATUS = SC_STATUS_RXOV_Msk;
        ...
        if(sc->STATUS & SC_STATUS_TXOV_Msk)
        {
            sc->STATUS = SC_STATUS_TXOV_Msk;
            dev->errno = SCLIB_ERR_WRITE;
            dev->op_state = SCLIB_OP_IDLE;
            SCLIB_ERR("SC%d: Tx Over Flow\n", num);
        }

        if(sc->STATUS & SC_STATUS_PEF_Msk)
        {
            sc->STATUS = SC_STATUS_PEF_Msk;
            dev->errno = SCLIB_ERR_PARITY_ERROR;
            sc->ALTCTL |= SC_ALTCTL_RXRST_Msk;
            if(dev->CardCapabilities.Protocol.Selected != SCLIB_PROTOCOL_T1)    // for ATR reception
                dev->op_state = SCLIB_OP_IDLE;
            SCLIB_ERR("SC%d: Rx Parity Error\n", num);
        }

        if(sc->STATUS & SC_STATUS_BEF_Msk)
        {
            sc->STATUS = SC_STATUS_BEF_Msk;
            sc->ALTCTL |= SC_ALTCTL_RXRST_Msk;
            dev->errno = SCLIB_ERR_READ;
            dev->op_state = SCLIB_OP_IDLE;
        ...
        if(sc->STATUS & SC_STATUS_FEF_Msk)
        {
            sc->STATUS = SC_STATUS_FEF_Msk;
            sc->ALTCTL |= SC_ALTCTL_RXRST_Msk;
            dev->errno = SCLIB_ERR_READ;
            dev->op_state = SCLIB_OP_IDLE;
        ...
        if(sc->STATUS & SC_STATUS_TXOV_Msk)
        {
            if(dev->op_state == SCLIB_OP_WRITE)
                sc->INTEN &= ~SC_INTEN_TXEIEN_Msk;
            sc->STATUS = (SC_STATUS_TXOV_Msk | SC_STATUS_TXRTYERR_Msk);
            sc->ALTCTL |= SC_ALTCTL_TXRST_Msk;         // Tx software reset
            dev->errno = SCLIB_ERR_WRITE;
            dev->op_state = SCLIB_OP_IDLE;
        ...
        if(sc->STATUS & SC_STATUS_RXOV_Msk)
        {
            sc->STATUS = (SC_STATUS_RXOV_Msk | SC_STATUS_RXRTYERR_Msk);
            dev->errno = SCLIB_ERR_READ;
            dev->op_state = SCLIB_OP_IDLE;
